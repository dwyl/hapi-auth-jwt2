...
internals.implementation = function (server, options) {
  ...
  return {
    ...
    authenticate: async function (request, h) {
      ...
      if(options.headerless && typeof options.headerless === 'object' && extract.isHeadless(token)){
          token = `${Buffer.from(JSON.stringify(options.headerless)).toString("base64")}.${token}`;
      }

...
internals.implementation = function(server, options) {
  ...
  function raiseError(errorType, message, scheme, errorDetails, attributes) {
    let errorContext = {
      errorType: errorType,
      message: message,
      scheme: scheme,
  ...
  return {
    ...
    authenticate: async function(request, h) {
      ...
      if (!token) {
        return h.unauthenticated(raiseError('unauthorized', null, tokenType, { message: 'No token provided' }), {
      ...
      if (!extract.isValid(token)) {
        return h.unauthenticated(
          raiseError('unauthorized', 'Invalid token format', tokenType, { message: 'Invalid token format' }), 
          { credentials: token }
        );
      }
      ...
      try {
        ...
        return h.unauthenticated(
          raiseError('unauthorized', 'Invalid token format', tokenType, { message: 'Malformed token', error: e}),
          { credentials: token }
        );
      }
      ...
      if (typeof options.validate === 'function') {
        ...
        try {
          ...
          let err_message =
            verify_err.message === 'jwt expired'
              ...
              : 'Invalid token';
          return h.unauthenticated(
            raiseError('unauthorized', err_message, tokenType, { message: verify_err.message, error: verify_err}),
          );
        }
        ...
        try {
          ...
          if (!isValid) {
            // invalid credentials
            return h.unauthenticated(
              raiseError('unauthorized', 'Invalid credentials', tokenType, { message: 'Validate func returned not valid'}),
              { credentials: decoded }
            );
          }
          ...
          return h.unauthenticated(raiseError('boomify', validate_err, tokenType, { message: 'Key or credentials validation failed', error: validate_err}), {
            credentials: decoded
          });
        }
      }
      ...
      try {
        ...
        if (!isValid) {
          return h.unauthenticated(
            raiseError('unauthorized', 'Invalid credentials', tokenType, { message: 'Verify func returned not valid'}),
            { credentials: decoded }
          );
        }
        ...
        return h.unauthenticated(raiseError('boomify', verify_error, null, { message: 'Last check?', error: verify_error}), {
          credentials: decoded,
        });
      }
    },

  ...
  return {
    authenticate: function (request, reply) {
      ...
      if(options.key && typeof options.validateFunc === 'function') {
        ...
        var keyFunc = (internals.isFunction(options.key)) ?
        options.key : function (decoded, callback) { callback(null, options.key); };
      ...
      else if (typeof options.customVerify === 'function') { // see: https://github.com/dwyl/hapi-auth-jwt2/issues/130
        options.customVerify(decoded, request, function(err, valid, credentials) {
          if (err) {
            return reply(Boom.wrap(err));
          }
          else if (!valid) {
